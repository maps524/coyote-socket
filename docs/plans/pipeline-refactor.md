...
| **5+6+7 (bundled)** — Bus + Transforms + drop `buttplug_links` | ✅ shipped | all subs A–G shipped — see substep table below |
| 8 — Introduce `InputSource` trait | ⏸ deferred | The four sources (T-Code, gamepad, buttplug, lovense) have divergent lifecycles; `ProcessingState::bus_write` already is the unifying boundary every source funnels through. Per CLAUDE.md ("don't introduce abstractions beyond what the task requires"), defer the trait until a fifth source actually needs the polymorphism. |
...
| F — Delete `buttplug/pipeline.rs`, `buttplug/state.rs`, most of `buttplug/types.rs` | ✅ shipped (`47f1039`, `7fddc0f`) | `git rm src-tauri/src/buttplug/pipeline.rs src-tauri/src/buttplug/state.rs`. Trimmed `buttplug/types.rs` down to `ButtplugFeatureType` + `ButtplugFeatureConfig` (descriptor-only). Moved canonical `ConstrictionMethod` into `transforms/mod.rs`. Dropped `Channel.buttplug_link`, `Channel.buttplug_state`, `set_buttplug_link_config`, `get_buttplug_feature_values`, `has_buttplug_input`, `BUTTPLUG_MAX_FEATURES`. Dropped `ButtplugLinksSettings::to_link_config`. Removed the `bp_config` write in `apply_channel_config_to_state`. |
| G.0 — Backend `resolved-update` event | ✅ shipped (`bcdb836`, `b4e70a5`) | New Tauri event emitted at the 10Hz device tick. `ResolvedUpdatePayload { timestamp_ms, channel_a, channel_b }` + `ChannelResolvedSnapshot` + `ResolvedSampleSnapshot` defined in `resolver.rs`. `Channel.last_intensity_sample` stash captures the bp:-path resolver output inside `get_next_waveform_data`. Engine-path Linked synthesizes from V2 ramp / V3 lookahead so the UI sees the device's actual transmitted value. |
| G.1 — Frontend resolved-state stream | ✅ shipped (`645f0d3`, `9073ffd`) | New `resolvedState.ts` store consumes `resolved-update` (RAF-smoothed). Per-parameter position indicators in `RangeSliderWithIndicator` now read `normalized_pre_range` straight from the resolver. Drops frontend curve evaluation (`applySourceTransform`/`applyCurve`/`applyMidpoint`/`smoothstep`) + `inputPosition.ts`. Follow-up: `is_static` field dropped from wire (frontend infers Static from `source_axis === undefined`); `indicatorOf` helper centralizes the gate; RAF-after-stop race guarded; `debug_assert!` added on bp:→engine-path-Linked stash invariant. |
| G.2 — Transforms editor UI | ✅ shipped (`69f92e3`, `f4f8cb9`) | New `TransformsEditor.svelte` component + `Transform` discriminated TS union (10 variants). Editor surfaces inside the existing source popover with list / add / reorder / delete. Backend gains `ParameterSourceSettings.transforms: Vec<TransformConfig>`. `convert_parameter_source` prefers editor-supplied transforms; falls back to legacy `buttplug_links_to_transforms` for old saves until G.3 retires it. Follow-up: fallback emits `log_warn!` for observability; non-`bp:` links show "transforms inactive" hint banner; empty modifier-axis fields render destructive border. |
| G.3 — `axis-update`→`bus-update` rename + buttplug-surface unification | ✅ shipped (`8b756c7`, `48600c6`, `cc7d3e4`) | G.3.0: `bus-update` event fires per-write inside `ProcessingState::bus_write`; new `inputBus.ts` store; InputMonitor migrates; `axis-update` + `buttplug-features` events + `emit_axis_update` + `emit_buttplug_features` + `get_buttplug_features` deleted. G.3.1: `ButtplugLinkPanel.svelte` (475 lines) deleted; `buttplug_links` schema + `ButtplugLinksSettings` + `ButtplugFeatureLinkSettings` + `ButtplugFeatureConfigSettings` deleted; legacy translation in `convert_parameter_source` deleted; `inputMode` ecosystem split + `effectiveInputMode` deleted; `settingsToButtplugLinks` + `buttplugLinksToSettings` helpers deleted. G.3.2: TransformsEditor's modifier-axis inputs gain a shared `<datalist>` populated from `inputBus.knownAxes`. G.3.3: dead `RangeSlider.svelte` (258 lines) deleted. |
