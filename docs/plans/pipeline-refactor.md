...
| E — Resolver rewrite to take `InputBusSnapshot` + pre-fetched modifiers | ⏳ pending | Removes the intensity short-circuit at the old `processing.rs:1546`. Buttplug-driven intensity now flows through curve / midpoint / range like every other source. Wires `ParameterLinkConfig.transforms` into the per-tick loop with pre-fetched modifiers. **Sub E must drop the `Channel.buttplug_link` consumer in the same commit it wires `transforms` (otherwise the resolver double-applies).** Validates the layered architecture end-to-end. |
| F — Delete `buttplug/pipeline.rs`, `buttplug/state.rs`, most of `buttplug/types.rs` | ⏳ pending | Per the deletion manifest. By this point nothing reads them; the substep is purely a `git rm`. Move `ConstrictionMethod` to `transforms/buttplug.rs` (currently re-exported from `crate::buttplug`). |
