...
//! endpoint changes. That is fine: cancelling it drops the whole `TcpStream`,
//! so there is no half-read frame to be left behind. It is specifically *not*
//! the same as putting `read_frame` in a losing `select!` arm, which would
//! consume a length prefix and then abandon its payload, desyncing a stream
//! that stays open. The read and write halves remain separate tasks inside a
